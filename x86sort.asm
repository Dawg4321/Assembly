...
_sort proc
		...
		mov ecx, 1				; i = 1
Loop1:  cmp ecx, [ebp + 12]
		jae FIN					; while i is less than length

		mov edx, ecx			; j = i
Loop2:	cmp edx, 0				; comparing j to zero
...
CAP1:	bt eax, 5				; testing if there is a capital in al. If there is a cap, the 6th bit will be clear
...
CAP2:	bt eax, 13				; testing if there is a capital in ah. If there is a cap, the 6th bit will be clear
...
CMP1:	cmp ah, al				; as both letters are now lowercase, the two letters can be compared
...
SCAP1:	cmp bl, 1				; check if bl to see if al needs to reset to a cap
...
SCAP2:	cmp bh, 1				; check if bh to see if ah needs to reset to a cap
...
SWAP:	mov [esi+edx], ah		; Swapping s[j] and s[j-1] 
		mov [esi+edx-1], al
...
NCAP:	mov bl, 0				; reset capital flag 1 to '0'
...
NOSWAP:	add ecx, 1				; i = i + 1	
		jmp Loop1				; continue back to top of Loop1

FIN:	mov esp, ebp			; deallocating ebp
